...
const resp=await fetchT(`${API_BASE}${endpoint}`,{method:'POST',headers,body:JSON.stringify({user_id:USER_ID,...body}),});if(resp.status===401||resp.status===403){console.error('[PROLOGUE] Auth error:',resp.status);const tg=window.Telegram?.WebApp;if(tg?.sendData){tg.sendData(JSON.stringify({action:'webapp_reconnect',webapp:'PROLOGUE',reason:resp.status===401?'session_expired':'invalid_init_data',}));setTimeout(function(){if(tg.close)tg.close();},1000);}
...
function closeLoreOverlay(){haptic();document.getElementById('loreOverlay').classList.remove('active');}
...
async function doFight(){try{document.getElementById('loading').style.display='flex';document.querySelector('#loading p').textContent='Preparando combate...';const result=await apiCall('/api/prologue/fight',{});if(result.combat_url||result.arena_url){window.__valdoria_transitioning=true;valdoriaSpaNav(result.combat_url||result.arena_url);}else{showError('Erro ao iniciar combate.');}}catch(e){showError('Erro ao iniciar combate.',e);}}
function onAftermathDone(){nextScreen(renderGate());}
async function onEnterCity(){try{document.getElementById('loading').style.display='flex';document.querySelector('#loading p').textContent='Entrando em Eldoria...';const body={gate_choice:choices.gate_choice||'',interaction_type:choices.interaction_type||'gate',aftermath_only:MODE==='aftermath',};if(choices.distract){body.distract=choices.distract;}
...
try{DATA=await apiCall('/api/prologue/init');if(window._prologueInitLoading){window._prologueInitLoading.hide();}
document.getElementById('loading').style.display='none';if(typeof ValdoriaAudio!=='undefined')ValdoriaAudio.play('prologue');if(DATA.mode==='aftermath'){addScreen(renderAftermath());goToScreen(0);return;}
