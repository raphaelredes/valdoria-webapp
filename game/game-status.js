/* ===================================================================
   GAME HUB — Character Details Tabbed Renderer
   Renders structured char_details data as a tabbed interface.
   Client-side tabs — no server roundtrip on tab switch.
   =================================================================== */
...
function renderCharDetails(container, data) {
    ...
    nameEl.className = "char-details-name";
    var badge = data.badge ? data.badge + " " : "";
    var nameTxt = badge + data.name + " Lv." + data.level + " " + data.class_name;
    if (data.subclass) nameTxt += " — " + data.subclass;
    nameEl.textContent = nameTxt;
    hdr.appendChild(nameEl);

    var metaEl = document.createElement("div");
    metaEl.className = "char-details-meta";
    var metaTxt = "🏷️ " + data.race;
    if (data.background) metaTxt += " | 📜 " + data.background;
    ...
    tabRow.className = "char-tabs";

    var savedTab = localStorage.getItem(_CHAR_TAB_STORAGE_KEY) || "overview";
    var panels = {};

    for (var i = 0; i < _CHAR_TABS.length; i++) {
        var t = _CHAR_TABS[i];
    ...
    // Tab panels
    panels["overview"] = _buildOverviewPanel(data);
    panels["attrs"] = _buildAttrsPanel(data);
    panels["combat"] = _buildCombatPanel(data);
    panels["feats"] = _buildFeatsPanel(data);

    for (var key in panels) {
        panels[key].className = "char-tab-panel" + (savedTab === key ? " active" : "");
...
function _buildOverviewPanel(data) {
    ...
    panel.appendChild(_makeBar("❤️", data.hp.cur, data.hp.max, _hpBarClass(data.hp.cur, data.hp.max)));
    panel.appendChild(_makeBar(data.mp.icon || "💧", data.mp.cur, data.mp.max, "mp"));
    var xpLabel = _abbreviate(data.xp.cur) + "/" + _abbreviate(data.xp.max) + " XP";
    panel.appendChild(_makeBarCustom("✨", data.xp.cur, data.xp.max, "xp", xpLabel));
    var hdLabel = data.hd.cur + "/" + data.hd.max + " (" + data.hd.die + ")";
    panel.appendChild(_makeBarCustom("🎲", data.hd.cur, data.hd.max, "hd", hdLabel));

    var resRow = document.createElement("div");
    resRow.className = "char-resources";
    resRow.appendChild(_makeResourceBadge("💰", data.gold, "GP"));
    ...
    if (data.conditions && data.conditions.length > 0) {
        ...
        condDiv.textContent = "⚠️ " + data.conditions.join(" · ");
...
function _makeBar(icon, cur, max, type) {
    var pct = max > 0 ? Math.round((cur / max) * 100) : 0;
    var row = document.createElement("div");
    row.className = "char-bar-row";
    var iconEl = document.createElement("span");
    iconEl.className = "char-bar-icon";
    iconEl.textContent = icon;
    row.appendChild(iconEl);
    var track = document.createElement("div");
    track.className = "char-bar-track";
    var fill = document.createElement("div");
    fill.className = "char-bar-fill " + type;
...
function _makeBarCustom(icon, cur, max, type, labelText) {
    var bar = _makeBar(icon, cur, max, type);
    bar.querySelector(".char-bar-label").textContent = labelText;
    return bar;
}
...
function _makeResourceBadge(icon, val, label) {
    ...
    badge.innerHTML = icon + " <span class=\"val\">" + val + "</span> " + label;
    return badge;
}
...
function _buildCombatPanel(data) {
    ...
    grid.appendChild(_makeCombatCard(
        "🛡️", "Defesa (CA)", c.ac.total,
        "10 " + dexSign + c.ac.dex_mod + "(DEX) + " + c.ac.equip_ac + "(Itens)"
    ));
    grid.appendChild(_makeCombatCard(
        "🎯", "Ataque", (c.atk.total >= 0 ? "+" : "") + c.atk.total,
        (c.atk.stat_mod >= 0 ? "+" : "") + c.atk.stat_mod + "(" + c.atk.stat + ") + " + c.atk.prof + "(Prof.) + " + c.atk.item_bonus + "(Item)"
    ));
    grid.appendChild(_makeCombatCard(
        "💥", "Dano", c.dmg.die + " " + (c.dmg.bonus >= 0 ? "+" : "") + c.dmg.bonus,
