...
function _fetchSocial(body) {if(window._dbg)console.debug('[SOCIAL-DBG] _fetchSocial action=' + (body&&body.action||'?') + ' apiBase=' + (window.S&&S.apiBase||'EMPTY').substring(0,40) + ' token=' + (window.S&&S.token?'YES('+S.token.substring(0,8)+')':'NO') + ' uid=' + (window.S&&S.uid||0));
    ...
    var _acS=new AbortController();var _tidS=setTimeout(function(){_acS.abort();},15000);return fetch(S.apiBase + '/api/game/social', {
        ...
        signal: _acS.signal,
    }).then(function(r) { clearTimeout(_tidS);if(window._dbg)console.debug('[SOCIAL-DBG] fetch resp status=' + r.status + ' ok=' + r.ok);if(r.status===401||r.status===403){console.warn('[SOCIAL] Auth error:',r.status);if(typeof vToast==='function')vToast('Sessão expirada. Feche e reabra.','warn',3000);return null;} if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); }).catch(function(e) {
...
function _buildChatMessages() {
    ...
    for (var i = 0; i < msgs.length; i++) {
        var m = msgs[i];
        if (m.system) { h += '<div class="social-msg social-msg--system">' + vEsc(m.text) + '</div>'; continue; }
        // Grouping: skip meta if same sender as previous non-system msg within 5min
        var prev = (i > 0) ? msgs[i - 1] : null;
        var grouped = false;
        if (prev && !prev.system && prev.uid && m.uid && prev.uid === m.uid) {
            try {
                var tA = prev.ts ? new Date(prev.ts).getTime() : 0;
                var tB = m.ts ? new Date(m.ts).getTime() : 0;
                if (tA && tB && (tB - tA) < 300000) grouped = true;
            } catch(e) { /* ignore */ }
        }
        var eCls = m.emote ? ' social-msg--emote' : '';
        var gCls = grouped ? ' social-msg--grouped' : '';
        h += '<div class="social-msg' + eCls + gCls + '">';
        if (!grouped) {
        ...
        h += '<span class="social-msg-text">' + _rpFormat(m.text) + '</span></div>';
    }
    // New messages pill (sticky at bottom of chat area)
    h += '<div class="social-new-msgs' + (_unreadCount > 0 ? ' visible' : '') + '" id="social-new-msgs"><span class="social-new-msgs-pill">↓ ' + _unreadCount + ' nova' + (_unreadCount !== 1 ? 's' : '') + '</span></div>';
    return h;
}
...
function _renderChatMessages(forceScroll) {
    ...
    if (_userScrolledUp && !forceScroll && !wasNear) {
        // User scrolled up — increment unread, update pill, preserve scroll
        _unreadCount++;
        var pill = document.getElementById('social-new-msgs');
        if (pill) { pill.className = 'social-new-msgs visible'; var sp = pill.querySelector('.social-new-msgs-pill'); if (sp) sp.textContent = '↓ ' + _unreadCount + ' nova' + (_unreadCount !== 1 ? 's' : ''); }
...
function _showMiniProfile(uid, anchorEl) {
    ...
    if (anchorEl) {
        ...
        var cardW = 200;
        var left = Math.max(4, rect.left - pRect.left);
        // Clamp to popup bounds (flip if overflows right edge)
        if (left + cardW > (pRect.width || 390) - 8) left = Math.max(4, (pRect.width || 390) - cardW - 8);
...
function _bindListeners() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (t) { e.preventDefault(); e.stopPropagation(); var tab = t.getAttribute('data-social-tab'); if (tab && tab !== _activeTab) { _activeTab = tab; _detailNpc = null; _detailData = null; _selectedPlayer = null; _userScrolledUp = false; _chatScrollBound = false; _emoteGridOpen = false; _unreadCount = 0; _unreadCount = 0; _dismissMiniProfile(); try { localStorage.setItem('valdoria_social_tab', tab); } catch(ex){console.warn('[SOCIAL]',ex);} _render(); } return; }
        ...
        var pillEl = e.target.closest('#social-new-msgs');
        if (pillEl) { e.preventDefault(); e.stopPropagation(); _unreadCount = 0; var area = document.getElementById('social-chat-area'); if (area) { area.scrollTop = area.scrollHeight; _userScrolledUp = false; } var p = document.getElementById('social-new-msgs'); if (p) p.className = 'social-new-msgs'; return; }
...
window.showSocialPopup = function(data) {if(window._dbg)console.debug('[SOCIAL-DBG] showSocialPopup() data keys=' + (data?Object.keys(data).join(','):'NULL') + ' chat=' + !!(data&&data.chat) + ' online=' + (data&&data.online?data.online.length:0));
    _cachedData = data || {};
...
window.openSocialPopup = function() {if(window._dbg)console.debug('[SOCIAL-DBG] openSocialPopup() called, vPopup=' + (typeof vPopup) + ', S.apiBase=' + (window.S&&S.apiBase||'EMPTY').substring(0,40) + ', S.token=' + (window.S&&S.token?'YES':'NO'));
    _closed = false;
    if (typeof vPopup !== "undefined") {
        if(window._dbg)console.debug('[SOCIAL-DBG] vPopup.show() calling');
        vPopup.show({
            id: "social-popup-overlay",
            header: "👥 Social",
