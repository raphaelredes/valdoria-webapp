...
/* Uses shared vPopup system. Content renderers kept, overlay managed by vPopup. */
...
(function () {
    ...
    function _dotCls(status) {
        return 'v-popup-dot v-popup-dot--' + status;
    }
    ...
    function _bar(pct, cls) {
        return '<div class="v-popup-bar-wrap">' +
            '<div class="v-popup-bar-fill v-popup-bar--' + cls + '" style="width:' + Math.max(2, pct) + '%"></div>' +
            '</div>';
    }
    ...
    function _renderBody(d) {
        ...
        h += '<span class="v-popup-label">HP</span>';
        ...
        if (d.mp) {
            ...
            h += '<span class="v-popup-label">MP</span>';
        ...
        h += '<span class="v-popup-label">\uD83C\uDF5E Ra\u00e7\u00f5es</span>';
        h += '<span class="v-popup-value">' + foodLbl + '</span>';
        h += '</div>';

        h += '<div class="v-popup-row">';
        h += '<span class="' + _dotCls(d.tent ? 'ok' : 'crit') + '"></span>';
        h += '<span class="v-popup-label">\u26FA Barraca</span>';
        ...
        h += '<span class="v-popup-label">\uD83E\uDDEA Po\u00e7\u00f5es</span>';
        ...
        if (d.party_size > 1) {
            h += '<div class="v-popup-row">';
            h += '<span class="' + _dotCls('ok') + '"></span>';
            h += '<span class="v-popup-label">\uD83D\uDC64 Grupo</span>';
            h += '<span class="v-popup-value">' + d.party_size + ' membros</span>';
            h += '</div>';
        }
        ...
        if (d.field_gear && d.field_gear.length > 0) {
            ...
            for (var i = 0; i < d.field_gear.length; i++) {
                ...
                h += '<div class="v-popup-row" style="font-size:var(--v-font-md)">';
                h += '<span class="' + _dotCls(gSt) + '"></span>';
                h += '<span class="v-popup-label">' + g.emoji + ' ' + g.name + star + '</span>';
                if (g.has) h += '<span class="v-popup-value v-popup-value--dim">' + g.bonus + '</span>';
                h += '</div>';
            }
        }
        ...
        // Biome tip
        if (d.biome_tip) {
            h += '<div class="v-popup-tip">' + d.biome_tip.emoji + ' <em>' + d.biome_tip.text + '</em></div>';
        }
        ...
        if (d.warnings && d.warnings.length > 0) {
            ...
            for (var w = 0; w < d.warnings.length; w++) {
                h += '<div class="v-popup-warn-item">' + (wm[d.warnings[w]] || d.warnings[w]) + '</div>';
            }
        }
        ...
        if (d.quick_equip && d.quick_equip.available) {
            h += '<div class="v-popup-divider"></div>';
            h += '<div class="v-popup-equip-info">';
            ...
            h += '<div class="v-popup-equip-detail">' + parts.join(' + ') + '</div>';
            if (!d.quick_equip.affordable) {
                h += '<div class="v-popup-equip-short">\u26A0\uFE0F Faltam ' + d.quick_equip.shortfall + ' PO</div>';
            }
            h += '</div>';
        }
    ...
    function _renderActions(d) {
        ...
        if (d.quick_equip && d.quick_equip.available && d.quick_equip.affordable) {
            h += '<button class="v-popup-btn v-popup-btn--equip" data-action="quick_equip">';
        ...
        if (d.warnings && (d.warnings.indexOf('no_food') !== -1 || d.warnings.indexOf('low_food') !== -1 || d.warnings.indexOf('no_tent') !== -1)) {
            shops += '<button class="v-popup-btn" data-action="market_interact_menu_tentmaker">\u26FA Bjorn (Seleiro)</button>';
        }
        if (d.warnings && d.warnings.indexOf('no_potions') !== -1) {
            shops += '<button class="v-popup-btn" data-action="market_interact_menu_alchemist">\uD83E\uDDEA Mirena (Alquimista)</button>';
        }
        if (shops) h += '<div class="v-popup-btn-row">' + shops + '</div>';

        // Low HP actions
        if (isLowHp) {
            var hpBtns = '';
            if (d.has_healing_potion) {
                hpBtns += '<button class="v-popup-btn" data-action="action_quick_potion">\uD83E\uDDEA Beber Po\u00e7\u00e3o</button>';
            }
            if (d.has_hit_dice) {
                hpBtns += '<button class="v-popup-btn" data-action="action_field_rest">\uD83D\uDCA4 Descanso Curto</button>';
            }
            if (hpBtns) h += '<div class="v-popup-btn-row">' + hpBtns + '</div>';
            h += '<button class="v-popup-btn" data-action="open_inn">\uD83C\uDFE8 Ir para Estalagem</button>';
        }

        // Main actions
        h += '<div class="v-popup-divider"></div>';
        h += '<button class="v-popup-btn v-popup-btn--primary" data-action="proceed">\u2694\uFE0F Prosseguir Mesmo Assim</button>';
        h += '<button class="v-popup-btn v-popup-btn--cancel" data-action="cancel">\uD83C\uDFD8\uFE0F Voltar para Eld\u00f3ria</button>';

        return h;
    }

    // --- Show overlay via vPopup ---
    window.showTravelPrep = function (data) {
        _currentData = data;
        if (typeof vPopup === 'undefined') { console.warn('[TRAVEL_PREP] vPopup not loaded'); return; }

        // Header style based on state
        var headerText, headerClass = '';
        if (data.equip_result === 'success') {
            headerText = '\u2705 Equipado para a Viagem!';
            headerClass = 'v-popup-header--success';
        } else if (data.equip_result === 'insufficient') {
            headerText = '\uD83D\uDCB0 Ouro Insuficiente';
            headerClass = 'v-popup-header--warning';
        } else {
            var hasLowHp = data.warnings && data.warnings.indexOf('low_hp') !== -1;
            headerText = '\u26A0\uFE0F Prepara\u00e7\u00e3o de Viagem';
        ...
        vPopup.show({
            ...
            onAction: function (action, btn) {
                if (action === 'proceed') {
                    vPopup.hide();
                    setTimeout(function () { if (typeof doAction === 'function') doAction('action_explore_confirmed'); }, 150);
                    return true;
                }
                ...
                return false; // default: close + doAction
            }
        });
    };
