...
window.showQuestPopup=function(data){var dOv=document.getElementById('quest-diary-overlay');var dBd=document.getElementById('quest-diary-body');if(dOv&&dOv.classList.contains('active')&&dBd){var hdr=dOv.querySelector('.v-popup-header');if(hdr)hdr.innerHTML='\u2694\uFE0F '+_esc(data.title||'Detalhes da Miss\u00e3o');dBd.innerHTML='';if(typeof renderQuestDetail==='function'){renderQuestDetail(dBd,data);}else{dBd.innerHTML=_renderBody(data);}var act=dOv.querySelector('.v-popup-actions');if(act){var h='';if(data.can_turnin&&data.turnin_cb){h+='<button class="v-popup-btn v-popup-btn--success" onclick="doAction(\''+data.turnin_cb+'\')">\ud83d\udcdc Entregar Miss\u00e3o</button>';}if(data.can_abandon&&data.abandon_cb){h+='<button class="v-popup-btn v-popup-btn--danger" onclick="doAction(\''+data.abandon_cb+'\')">\u274c Abandonar</button>';}h+='<button class="v-popup-btn v-popup-btn--cancel" onclick="doAction(\'action_quests\')">\ud83d\udcdd Voltar \u00e0s Miss\u00f5es</button>';act.innerHTML=h;}return;}if(typeof vPopup==='undefined'){console.warn('[QUEST-POPUP] vPopup not loaded');return;}
...
var hdr=overlay.querySelector('.v-popup-header');if(hdr)hdr.innerHTML='\ud83d\udcdc Miss\u00f5es';var actEl=overlay.querySelector('.v-popup-actions');if(actEl)actEl.innerHTML='<button class="v-popup-btn v-popup-btn--cancel" onclick="hideQuestDiaryPopup()">\ud83c\udfe0 Voltar</button>';body.innerHTML='';if(typeof renderQuestDiary==='function'){renderQuestDiary(body,data);}else{body.innerHTML='<div style="text-align:center;color:var(--v-text-dim);padding:20px;">'
+'Carregando missões...</div>';console.error('[QUEST-POPUP] renderQuestDiary not available');}
overlay.style.display='flex';requestAnimationFrame(function(){overlay.classList.add('active');});};window.hideQuestDiaryPopup=function(){var overlay=document.getElementById('quest-diary-overlay');if(!overlay)return;overlay.classList.add('hiding');overlay.classList.remove('active');setTimeout(function(){overlay.style.display='none';overlay.classList.remove('hiding');},300);};window.showQuestAbandonPopup=function(data){var dOv=document.getElementById('quest-diary-overlay');if(dOv&&dOv.classList.contains('active')){var hdr=dOv.querySelector('.v-popup-header');if(hdr)hdr.innerHTML='\u26a0\ufe0f Abandonar Miss\u00e3o';var body=document.getElementById('quest-diary-body');if(body){var wt=data.is_daily?'Ao desistir, tudo que foi feito se perder\u00e1. Tarefas di\u00e1rias abandonadas contam para o limite do dia.':'Ao desistir, voc\u00ea perder\u00e1 tudo que conquistou nesta jornada. Poder\u00e1 retom\u00e1-la mais tarde.';body.innerHTML='<div style="text-align:center;margin:20px 0 12px;"><span style="font-size:32px;">\u26a0\ufe0f</span></div>'+'<div style="text-align:center;font-size:14px;color:var(--v-text);margin-bottom:12px;">\ud83d\udcdc <b>'+_esc(data.title)+'</b></div>'+'<div style="font-size:13px;color:var(--v-text-dim);text-align:center;line-height:1.5;">'+wt+'</div>';}var act=dOv.querySelector('.v-popup-actions');if(act){act.innerHTML='<button class="v-popup-btn v-popup-btn--danger" onclick="doAction(\''+data.confirm_cb+'\')">\u2705 Sim, Abandonar</button>'+'<button class="v-popup-btn v-popup-btn--cancel" onclick="doAction(\'action_quests\')">\ud83d\udd19 Cancelar</button>';}return;}if(typeof vPopup==='undefined'){console.warn('[QUEST-POPUP] vPopup not loaded');return;}
