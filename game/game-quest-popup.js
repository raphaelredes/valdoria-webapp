...
h+='<button class="v-popup-btn v-popup-btn--cancel" data-action="'+(backAction||'cancel')+'">'+(backLabel||'Fechar')+'</button>';return h;}
...
var diaryOv=document.getElementById('quest-diary-overlay');var inDiary=diaryOv&&diaryOv.classList.contains('active');var oid=inDiary?'quest-diary-overlay':'quest-popup-overlay';var bAct=inDiary?'action_quests':'cancel';var bLbl=inDiary?'📝 Voltar às Missões':'Fechar';
vPopup.show({id:oid,header:'⚔️ '+vEsc(data.title||'Detalhes da Missão'),bodyEl:bodyEl,actions:_questActionHtml(data,bAct,bLbl),onAction:inDiary?_diaryNav:null});};
...
vPopup.show({id:'quest-diary-overlay',header:'📜 Missões',bodyEl:bodyEl,actions:'<button class="v-popup-btn v-popup-btn--cancel" data-action="cancel">Fechar</button>'});};
