(function(){'use strict';var _cachedData=null;var _currentView='list';var _activeFilter='all';function _esc(text){if(!text)return'';return String(text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function _renderListBody(d){var h='';h+='<div class="qb-status">';h+='<span class="qb-rep">\u2694\uFE0F '+_esc(d.attitude)+' ('+(d.rep||0)+')</span>';h+='<span class="qb-daily">\ud83d\udccb '+_esc(d.daily)+' Di\u00e1rias</span>';h+='</div>';if(d.active_count>0){h+='<div class="qb-active-count">\ud83d\udccc '+d.active_count+' em andamento</div>';}
...
if(d.flavor){h+='<div class="qb-flavor">'+_esc(d.flavor)+'</div>';}
return h;}
function _renderListActions(){return'<button class="v-popup-btn v-popup-btn--cancel" data-action="cancel">'
+'\ud83c\udfe0 Voltar</button>';}
function _renderDetailBody(d,q){var h='';h+='<div style="text-align:center;margin-bottom:var(--v-space-sm);">';h+='<span style="font-size:28px;">'+(q.npc_icon||'\ud83d\udcdc')+'</span>';h+='</div>';if(q.npc){h+='<div style="font-size:var(--v-font-sm);color:var(--v-text-dim);text-align:center;margin-bottom:var(--v-space-md);">';h+='\ud83d\udcdc Solicitante: '+_esc(q.npc)+'</div>';}
var catLabel=q.cat==='daily'?'Di\u00e1ria':(q.cat==='story'?'Hist\u00f3ria':'Aventura');h+='<div style="display:flex;justify-content:center;gap:var(--v-space-md);margin-bottom:var(--v-space-md);">';h+='<span class="qb-cat-badge qb-cat-'+(q.cat||'daily')+'">'+catLabel+'</span>';h+='<span style="font-size:var(--v-font-sm);color:var(--v-text-dim);">'+(q.diff||'')+' N\u00edvel '+(q.min_lv||1)+'</span>';h+='</div>';if(q.desc){h+='<div class="v-popup-desc">'+_esc(q.desc)+'</div>';}
if(q.objs&&q.objs.length>0){h+='<div class="v-popup-section-label">\ud83c\udfaf Objetivos</div>';for(var i=0;i<q.objs.length;i++){h+='<div style="font-size:var(--v-font-md);color:var(--v-text);padding:2px 0;">';h+=(i+1)+'. <em>'+_esc(q.objs[i])+'</em></div>';}}
h+='<div class="v-popup-divider"></div>';h+='<div class="qb-detail-rewards">';h+='<div class="v-popup-section-label">\ud83d\udc8e Recompensas</div>';var rew='';if(q.gold)rew+='<span style="margin-right:var(--v-space-lg);">\ud83d\udcb0 '+q.gold+' GP</span>';if(q.xp)rew+='<span>\u2728 '+q.xp+' XP</span>';h+='<div style="font-size:var(--v-font-md);color:var(--v-text);">'+rew+'</div>';h+='</div>';if(q.inv==='success'||q.inv==='crit_success'){if(q.hint){h+='<div class="qb-check-result qb-check-success">';h+='\ud83d\udd0d '+_esc(q.hint)+'</div>';}}else if(q.inv==='fail'){h+='<div class="qb-check-result qb-check-fail">';h+='\ud83d\udd0d Nada al\u00e9m do \u00f3bvio.</div>';}
if(q.neg==='success'||q.neg==='crit_success'){if(q.neg_bonus){h+='<div class="qb-check-result qb-check-success">';h+='\ud83d\udcac Negocia\u00e7\u00e3o: '+_esc(q.neg_bonus)+'</div>';}}else if(q.neg==='fail'){h+='<div class="qb-check-result qb-check-fail">';h+='\ud83d\udcac Negocia\u00e7\u00e3o recusada.</div>';}
...
+'O papel ainda cheira a tinta fresca.</div>';h+='<div style="margin-top:var(--v-space-md);">';h+='\ud83d\udcdc <strong>'+_esc(a.title)+'</strong>';h+='</div>';if(a.obj){h+='<div style="font-size:var(--v-font-md);color:var(--v-text-dim);margin-top:var(--v-space-xs);">'
+'\ud83c\udfaf '+_esc(a.obj)+'</div>';}
...
+'N\u00e3o foi poss\u00edvel aceitar</div>';h+='</div>';h+='<div class="v-popup-desc">'+_esc(a.reason||'Requisitos n\u00e3o atendidos.')+'</div>';}
return h;}
...
header='\ud83d\udcdc '+_esc(quest.title);body=_renderDetailBody(d,quest);actions=_renderDetailActions(d,quest);}else{header='\ud83d\udccb Quadro de Miss\u00f5es';body=_renderListBody(d);actions=_renderListActions();}
