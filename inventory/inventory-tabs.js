...
function cycleSort(){const idx=SORT_CYCLE.indexOf(sortMode);sortMode=SORT_CYCLE[(idx+1)%SORT_CYCLE.length];try{localStorage.setItem('valdoria_inv_sort',sortMode);}catch(e){}haptic('light');renderTab();}
function setFilter(f){activeFilter=f;try{localStorage.setItem('valdoria_inv_filter',activeFilter);}catch(e){}renderTab();}
...
function _calcItemScore(it,slot){if(!it)return 0;let score=0;if(slot==='main_hand'||slot==='off_hand'){score+=(it.b||0)*10;if(it.dd){const dm=it.dd.match(/(\d+)d(\d+)/);if(dm)score+=parseInt(dm[1])*(parseInt(dm[2])+1)/2;}}
