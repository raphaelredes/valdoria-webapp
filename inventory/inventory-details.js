...
var GEM_UPGRADE_MAP={'Turquesa':'Ametista','Quartzo':'Jade','\u00d4nix':'P\u00e9rola','Ametista':'Rubi','Jade':'Safira','P\u00e9rola':'Esmeralda',};var GEM_UPGRADE_COST=3;function getGemFusions(){var gemCounts={};localInv.forEach(function(inv){if(inv.q<=0)return;var it=getItemData(inv.n);if(!it||!(it.t||[]).some(function(t){return t==='gem'||t==='socketable';}))return;if(!GEM_UPGRADE_MAP[inv.n])return;gemCounts[inv.n]=(gemCounts[inv.n]||0)+inv.q;});var fusions=[];for(var gem in gemCounts){if(gemCounts[gem]>=GEM_UPGRADE_COST){fusions.push({from:gem,to:GEM_UPGRADE_MAP[gem],count:gemCounts[gem]});}}
return fusions;}
