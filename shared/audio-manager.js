...
console.log('[AUDIO] play()',trackKey,'unlocked:',_unlocked,'muted:',_muted,'musicMuted:',_musicMuted,'current:',_currentTrack);
...
audio.addEventListener('volumechange',()=>{if(audio!==_audio||_fading)return;const newVol=audio.volume;if(Math.abs(newVol-_volume)>0.01&&!_muted){_volume=Math.max(0,Math.min(1,newVol));localStorage.setItem(VOLUME_KEY,_volume.toString());_syncUI();}});audio.play().then(()=>{_unlockAttempts=0;_crossfadeIn(audio);_syncUI();}).catch(e=>{_pendingTrack=trackKey;if(e.name==='NotAllowedError'){console.log('[AUDIO] Autoplay blocked for:',trackKey,'- awaiting gesture');_unlocked=false;_pendingTrack=trackKey;_registerUnlockListeners();_tryMutedAutoplay(trackKey);}else if(e.name==='AbortError'){console.debug('[AUDIO] Play interrupted (AbortError) for:',trackKey);}else{console.warn('[AUDIO] Play error:',e.name,e.message);if(_retryTimer)clearTimeout(_retryTimer);var _rc=(audio._retryCount||0)+1;if(_rc<=3){_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},Math.min(500*Math.pow(2,_rc-1),2000));}}});}
...
function _haptic(type){try{var tg=window.Telegram&&window.Telegram.WebApp;if(tg&&tg.HapticFeedback)tg.HapticFeedback.impactOccurred(type||'light');}catch(e){}}
let _volRafId=null;function _smoothVol(audio,target){if(_volRafId)cancelAnimationFrame(_volRafId);var current=audio.volume;var diff=target-current;if(Math.abs(diff)<0.01){try{audio.volume=target;}catch(e){}return;}
var step=diff*0.3;function tick(){current+=step;step*=0.7;if(Math.abs(target-current)<0.005){try{audio.volume=target;}catch(e){}
...
function _dismissHint(btn){if(_hintDismissed)return;_hintDismissed=true;try{localStorage.setItem(HINT_KEY,'1');}catch(e){}
...
function toggleMusic(){_musicMuted=!_musicMuted;try{localStorage.setItem(MUSIC_MUTED_KEY,_musicMuted?'1':'0');}catch(e){}if(_musicMuted){if(_audio)_audio.volume=0;_muted=true;try{localStorage.setItem(STORAGE_KEY,'1');}catch(e){}}else{_muted=false;try{localStorage.setItem(STORAGE_KEY,'0');}catch(e){}if(_audio&&!_audio.paused){_audio.volume=_volume;}else if(_pendingTrack||_currentTrack){_playTrack(_pendingTrack||_currentTrack);}}
