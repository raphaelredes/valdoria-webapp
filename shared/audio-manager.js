...
}).catch(function(e){if(window._dbg)console.debug('[AUDIO]',e.name);});}
...
newAudio.play().then(()=>{_crossfadeOut(oldAudio);_crossfadeIn(newAudio);_audio=newAudio;_looping=false;}).catch(()=>{oldAudio.currentTime=0;oldAudio.play().catch(function(e){if(window._dbg)console.debug('[AUDIO]',e.name);});_looping=false;});}
...
function playSFX(trackKey){if(_muted||_sfxMuted||!_unlocked)return;if(window.vReducedMotion)return;const file=_pickVariant(trackKey);if(!file)return;const url=AUDIO_BASE+file;const sfx=_getSfxAudio(url);sfx.volume=Math.min(1,_sfxVolume*1.5);sfx.play().catch(function(e){if(window._dbg)console.debug('[AUDIO]',e.name);});}
...
function playBiome(biome){const biomeMap={'plains':'city','forest':'forest','swamp':'swamp','mountain':'mountain','cave':'dungeon','desert':'desert','snow':'snow','volcanic':'dungeon','city_gates':'city','ocean':'ocean','coastal':'ocean','ruins':'ancient_ruins','underground':'dungeon',};const track=biomeMap[biome]||'forest';play(track);}
