...
var _vh=function(){if(audio!==_audio||_fading)return;if(audio.muted||_mutedAutoplay)return;var newVol=audio.volume;if(Math.abs(newVol-_volume)>0.01&&!_muted){_volume=Math.max(0,Math.min(1,newVol));try{localStorage.setItem(VOLUME_KEY,_volume.toString());}catch(e){console.warn('[AUDIO_MANAGER]',e);}_syncUI();}};audio._volHandler=_vh;audio.addEventListener('volumechange',_vh);audio.play().then(()=>{_unlockAttempts=0;if(_analyserNode)_connectAnalyser(audio);_crossfadeIn(audio);_syncUI();}).catch(e=>{_pendingTrack=trackKey;if(e.name==='NotAllowedError'){_unlocked=false;_pendingTrack=trackKey;_registerUnlockListeners();_tryMutedAutoplay(trackKey);}else if(e.name==='AbortError'){console.warn('[AUDIO] AbortError on play, scheduling retry for:',trackKey);if(_retryTimer)clearTimeout(_retryTimer);var _arc=(audio._abortRetry||0)+1;audio._abortRetry=_arc;if(_arc<=3){_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},500*_arc);}}else{console.warn('[AUDIO] Play error:',e.name,e.message);if(_retryTimer)clearTimeout(_retryTimer);var _rc=(audio._retryCount||0)+1;audio._retryCount=_rc;if(_rc<=3){_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},Math.min(500*Math.pow(2,_rc-1),2000));}}});}
