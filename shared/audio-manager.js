...
function _playTrack(trackKey){const file=_pickVariant(trackKey);if(!file)return;const url=AUDIO_BASE+file;if(_audio&&!_audio.paused){_crossfadeOut(_audio);}
...
btn.addEventListener('click',(e)=>{e.stopPropagation();_dismissHint(btn);_togglePopup();});_createPopup();const mo=new MutationObserver(()=>{_closePopup();if(!document.getElementById('va-btn')&&_isEmbedded){const el=document.getElementById('footer-quick');if(el&&el.children.length>0){_isEmbedded=false;_embedInFooter(el);}}});mo.observe(footerEl,{childList:true});_showFirstTimeHint(btn);}
