...
function init(){_muted=localStorage.getItem(STORAGE_KEY)==='1';/* sessão #23 (2026-05-22): reset 1x para DEFAULT_VOLUME se preset version mudou. User pediu volume default mais baixo — força nova baseline pra todos os players. */try{var presetV=localStorage.getItem(VOLUME_PRESET_VERSION_KEY);if(presetV!==VOLUME_PRESET_VERSION){localStorage.setItem(VOLUME_KEY,String(DEFAULT_VOLUME));localStorage.setItem(SFX_VOLUME_KEY,String(DEFAULT_VOLUME));localStorage.setItem(VOLUME_PRESET_VERSION_KEY,VOLUME_PRESET_VERSION);console.info('[AUDIO] Volume preset migrated to v'+VOLUME_PRESET_VERSION+' (vol='+DEFAULT_VOLUME+')');}}catch(e){console.warn('[AUDIO_MANAGER] preset migration:',e);}const savedVol=parseFloat(localStorage.getItem(VOLUME_KEY));if(!isNaN(savedVol)&&savedVol>=0&&savedVol<=1){_volume=savedVol;}
...
_looping=false;if(_analyserCtx&&_analyserCtx.state==='suspended'){try{_analyserCtx.resume();}catch(e){console.warn('[AUDIO] ctx resume:',e);}}const audio=new Audio(url);audio.volume=0;audio.preload='auto';_audio=audio;_currentTrack=trackKey;if(!NO_LOOP_TRACKS.includes(trackKey)){audio.addEventListener('timeupdate',_onTimeUpdate);}
/* FIX 2026-05-04 USER REQUEST: volumechange listener REMOVIDO. Antes, listener
   overwrite _volume com valores intermediários do _smoothVol (ticks RAF), causando
   drift e perda do valor exato escolhido pelo jogador. setVolume() é a ÚNICA fonte
   de verdade pra _volume + localStorage. Mantém _volHandler como noop pra compat
   com _crossfadeOut que tenta removeEventListener. */
var _vh=function(){};audio._volHandler=_vh;audio.play().then(()=>{_unlockAttempts=0;_abortCount=0;if(_analyserNode)_connectAnalyser(audio);_crossfadeIn(audio);_syncUI();}).catch(e=>{_pendingTrack=trackKey;if(e.name==='NotAllowedError'){_unlocked=false;_pendingTrack=trackKey;_registerUnlockListeners();_tryMutedAutoplay(trackKey);}else if(e.name==='AbortError'){_abortCount=(_abortCount||0)+1;if(_abortCount<=3){console.warn('[AUDIO] AbortError on play, retry',_abortCount,'/3 for:',trackKey);if(_retryTimer)clearTimeout(_retryTimer);_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},500*_abortCount);}else{console.warn('[AUDIO] AbortError max retries reached, waiting for user gesture for:',trackKey);_unlocked=false;_registerUnlockListeners();}}else{console.warn('[AUDIO] Play error:',e.name,e.message);if(_retryTimer)clearTimeout(_retryTimer);var _rc=(audio._retryCount||0)+1;audio._retryCount=_rc;if(_rc<=3){_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},Math.min(500*Math.pow(2,_rc-1),2000));}}});}
...
function _crossfadeIn(audio){if(!audio)return;_fading=true;const target=_volume*0.5;/*2026-05-22 user cap musica 50% mesmo no slider max*/const steps=20;const stepMs=CROSSFADE_MS/steps;const increment=target/steps;let current=0;const timer=setInterval(()=>{current+=increment;if(current>=target||audio!==_audio){try{audio.volume=(audio===_audio)?target:0;}catch(e){console.warn('[AUDIO_MANAGER]',e);}
...
function _injectCSS(){const style=document.createElement('style');style.textContent=`
            ...
            /* 2026-07-03: emoji ♫/⚔ -> ui/*.webp (SUPREME RULE: zero emoji player-visible).
               Estado mute indicado por OPACIDADE (img nao aceita 'color'). */
            .va-popup-icon-img { width: 18px; height: 18px; object-fit: contain; opacity: 0.4; transition: opacity var(--v-transition, 0.2s); }
            .va-popup-icon.unmuted .va-popup-icon-img { opacity: 1; }
...
function _updatePopupUI(){/* USER REQUEST 2026-05-04: track name display removido. */const mi=document.getElementById('va-music-icon');const ms=document.getElementById('va-music-slider');const mp=document.getElementById('va-music-pct');const mv=(_muted||_musicMuted)?0:Math.round(_volume*100);if(mi)mi.classList.toggle('unmuted',!_musicMuted&&!_muted);if(ms){ms.value=String(mv);_updateSliderFill(ms,mv);}
...
/* task #87 (2026-05-20) — public embedInFooter API. Permite que páginas que
   ...
   onde renderFooter() roda após o setTimeout 3s do _tryEmbedInFooter. */
embedInFooter:function(footerEl){if(!footerEl)footerEl=document.getElementById('footer-quick');if(!footerEl||footerEl.children.length===0)return false;var floatEl=document.getElementById('va-float');if(floatEl)floatEl.remove();var oldBtn=document.getElementById('va-btn');if(oldBtn)oldBtn.remove();_isEmbedded=false;_embedInFooter(footerEl);return true;}
};})();if(typeof document!=='undefined'){if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',ValdoriaAudio.init);}else{ValdoriaAudio.init();}
window.addEventListener('pagehide',function(){try{var ct=(typeof ValdoriaAudio.getCurrentTrack==='function')?ValdoriaAudio.getCurrentTrack():'';if(ct){try{localStorage.setItem('valdoria_audio_resume',ct);}catch(e){console.warn('[AUDIO_MANAGER]',e);}try{localStorage.setItem('valdoria_audio_resume_ts',String(Date.now()));}catch(e){console.warn('[AUDIO_MANAGER]',e);}}/* FIX 2026-05-04: belt-and-suspenders save dos volumes em pagehide. Garante persistência mesmo se algum caminho não-canônico mudou _volume sem chamar setVolume. */try{localStorage.setItem(VOLUME_KEY,_volume.toString());}catch(e){}try{localStorage.setItem(SFX_VOLUME_KEY,_sfxVolume.toString());}catch(e){}try{localStorage.setItem(STORAGE_KEY,_muted?'1':'0');}catch(e){}try{localStorage.setItem(MUSIC_MUTED_KEY,_musicMuted?'1':'0');}catch(e){}try{localStorage.setItem(SFX_MUTED_KEY,_sfxMuted?'1':'0');}catch(e){}}catch(e){console.warn('[AUDIO_MANAGER]',e);}});}
