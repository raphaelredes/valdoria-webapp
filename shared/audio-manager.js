...
audio.addEventListener('volumechange',()=>{if(audio!==_audio||_fading)return;const newVol=audio.volume;if(Math.abs(newVol-_volume)>0.01&&!_muted){_volume=Math.max(0,Math.min(1,newVol));localStorage.setItem(VOLUME_KEY,_volume.toString());_syncUI();}});audio.play().then(()=>{_unlockAttempts=0;_crossfadeIn(audio);_syncUI();}).catch(e=>{_pendingTrack=trackKey;if(e.name==='NotAllowedError'){console.warn('[AUDIO] Autoplay blocked, awaiting user gesture');_unlocked=false;_unlockAttempts++;if(_unlockAttempts<_MAX_UNLOCK_ATTEMPTS){_registerUnlockListeners();}else{_removeUnlockListeners();}_tryMutedAutoplay(trackKey);}else if(e.name==='AbortError'){console.debug('[AUDIO] Play interrupted (AbortError) for:',trackKey);}else{console.warn('[AUDIO] Play error:',e.name,e.message);if(_retryTimer)clearTimeout(_retryTimer);var _rc=(audio._retryCount||0)+1;if(_rc<=3){_retryTimer=setTimeout(function(){_retryTimer=null;if(_pendingTrack===trackKey&&!_muted&&!_musicMuted){_playTrack(trackKey);}},Math.min(500*Math.pow(2,_rc-1),2000));}}});}
