...
var card=overlay.querySelector('.v-popup-card');if(opts.size==='mini'){if(card)card.classList.add('v-popup-card--mini');}else{if(card)card.classList.remove('v-popup-card--mini');}
overlay.style.zIndex=_zBase;overlay.style.display='flex';overlay.classList.remove('hiding');void overlay.offsetWidth;overlay.classList.add('active');_busy=false;if(opts.onReady&&typeof opts.onReady==='function'){opts.onReady(overlay);}var _body=overlay.querySelector('.v-popup-body');if(_body){var _updFade=function(){if(_body.scrollHeight>_body.clientHeight+2){_body.classList.add('has-scroll');if(_body.scrollTop+_body.clientHeight>=_body.scrollHeight-4){_body.classList.add('at-bottom');}else{_body.classList.remove('at-bottom');}}else{_body.classList.remove('has-scroll');_body.classList.remove('at-bottom');}};_updFade();_body.onscroll=_updFade;}
