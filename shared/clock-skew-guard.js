...
(function () {
  ...
  function _showOverlay(skewMs, serverMs) {
    ...
    ov.innerHTML = ''
      ...
      + '    <p class="vcsg-step"><b>Após ajustar:</b> reinicie o celular.</p>'
      + '  </div>'
      + '  <p class="vcsg-warning">⚠️ Se ignorar, pode ter erros de "conexão não privada" e não conseguir abrir áreas do jogo.</p>'
      + '  <p class="vcsg-tip">💡 Se o erro persistir mesmo com data certa, digite <b>/ajuda</b> no chat do bot — pode ser VPN/antivírus interceptando.</p>'
  ...
  function _injectCSS() {
    ...
    st.textContent = ''
      ...
      + '.vcsg-tip{font-size:10px;color:#a09484;font-style:italic;text-align:center;'
      + '  margin:0 0 14px;line-height:1.4}'
      + '.vcsg-tip b{color:#c4953a;font-style:normal}'
