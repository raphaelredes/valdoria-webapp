...
async function _levelupTransitionBack(){const h={'Content-Type':'application/json','Authorization':'Bearer '+token};if(window.Telegram?.WebApp?.initData)h['X-Telegram-Init-Data']=Telegram.WebApp.initData;h['X-Idempotency-Key']=crypto.randomUUID();try{const r=await fetchT(apiFallback+'/api/webapp/transition',{method:'POST',headers:h,body:JSON.stringify({from:'levelup',to:returnTo,user_id:parseInt(userId),payload:{}})});if(r.status===401||r.status===403){console.error('[LEVELUP] Auth error on transition:',r.status);if(tg?.sendData){tg.sendData(JSON.stringify({action:'webapp_error_close',webapp:'LEVELUP',reason:'session_expired'}));return;}}
const d=await r.json();if(d.url){valdoriaSpaNav(d.url);return;}}catch(e){console.error('[LEVELUP] transition error:',e);}
try{if(tg)tg.close();}catch(e){console.warn('[LEVELUP] tg.close:',e);}}
const screens=[];let cur=0;let asiMode='asi';let isTransitioning=false;function decodeBase64Utf8(b64){const raw=b64.replace(/-/g,'+').replace(/_/g,'/');const binary=atob(raw);const bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);return new TextDecoder().decode(bytes);}
...
token=params.get('token')||'';apiFallback=params.get('api')||'';userId=params.get('uid')||'';if(window.SessionHeartbeat&&apiFallback&&token&&userId){SessionHeartbeat.init({apiBase:apiFallback,token:token,uid:parseInt(userId)||0});}
...
function getAvailableSkills(){const pool=CLASS_SKILLS[P.hero_class]||{};const result=[];for(const[lv,skills]of Object.entries(pool)){if(parseInt(lv)<=P.level){for(const s of skills){if(!P.acquired_skills.includes(s.id)){result.push({...s,level:parseInt(lv)});}}}}
return result;}
function _showValidation(msg){console.warn('[LEVELUP]',msg);hapticNotify('error');if(window.ValdoriaErrors){ValdoriaErrors.showToast(msg,3000);}else{const t=document.getElementById('toast');t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),3000);}}
...
let _submitted=false;function submit(){if(_submitted)return;_submitted=true;haptic('medium');const asiChoices=[];for(const[attr,count]of Object.entries(sel.asi)){for(let i=0;i<count;i++)asiChoices.push(attr);}
const payload={action:'levelup_complete',token:token,asi_choices:asiChoices,skill_choices:sel.skills,subclass:sel.subclass,feat:sel.feat,feat_stat:sel.featStat,};const btn=document.getElementById('btnNext');btn.disabled=true;btn.textContent='Enviando...';document.getElementById('loadingOverlay').classList.add('active');try{if(apiFallback){payload.user_id=parseInt(userId);const _lh={'Content-Type':'application/json','Authorization':'Bearer '+token};if(window.Telegram?.WebApp?.initData){_lh['X-Telegram-Init-Data']=Telegram.WebApp.initData;}
_lh['X-Idempotency-Key']=crypto.randomUUID();fetchT(apiFallback+'/api/levelup/apply',{method:'POST',headers:_lh,body:JSON.stringify(payload)}).then(res=>{if(res.status===401||res.status===403){console.error('[LEVELUP] Auth error on apply:',res.status);document.getElementById('loadingOverlay').classList.remove('active');if(tg?.sendData){tg.sendData(JSON.stringify({action:'webapp_error_close',webapp:'LEVELUP',reason:'session_expired'}));return null;}
showFatalError('Sessão expirada. Feche e reabra.');return null;}
return res.json();}).then(data=>{if(!data)return;if(data.status==='saved'||data.ok){if(data.return_url){valdoriaSpaNav(data.return_url);}else if(returnTo&&apiFallback&&token){_levelupTransitionBack();}else if(tg&&tg.close){tg.close();}else{alert('Evolucao confirmada!');}}else{document.getElementById('loadingOverlay').classList.remove('active');btn.disabled=false;btn.textContent='Confirmar';showFatalError('Erro ao aplicar evolucao: '+(data.error||'Desconhecido'));}}).catch(e=>{document.getElementById('loadingOverlay').classList.remove('active');btn.disabled=false;btn.textContent='Confirmar';showFatalError('Erro de conexao com servidor.',e);});}
else if(tg&&tg.sendData){setTimeout(()=>{tg.sendData(JSON.stringify(payload));setTimeout(()=>{try{tg.close();}catch(e){console.warn('[LEVELUP] tg.close:',e);}},300);},300);setTimeout(()=>{if(document.visibilityState!=='hidden'){document.getElementById('loadingOverlay').classList.remove('active');btn.disabled=false;btn.textContent='Confirmar';}},5000);}else{console.log('Level-up payload:',JSON.stringify(payload,null,2));setTimeout(()=>{document.getElementById('loadingOverlay').classList.remove('active');btn.disabled=false;alert('Evolucao confirmada! (modo teste)\n\n'+JSON.stringify(payload,null,2));},800);}}catch(e){document.getElementById('loadingOverlay').classList.remove('active');btn.disabled=false;showFatalError('Erro ao enviar dados. Tente novamente.',e);}}