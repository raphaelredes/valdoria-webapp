...
var _spaP = window.__spaRouteParams || {};
        ...
        (function init() {
            ...
            if (typeof ValdoriaAudio !== 'undefined') ValdoriaAudio.play('levelup');
            // Hide initial loading with narrative delay
            if (window._lvlInitLoading) window._lvlInitLoading.hide();
        })();
        ...
        function showFatalError(msg, err = null) {
            ...
            // Use shared overlay if available, fallback to local
            if (window.ValdoriaErrors) {
                ...
                if (apiFallback) {
                    ...
                    fetchT(apiFallback + '/api/levelup/apply', {
                        ...
                        if (res.status === 401 || res.status === 403) {
                            ...
                            if (tg?.sendData) {
                                tg.sendData(JSON.stringify({ action: 'webapp_error_close', webapp: 'LEVELUP', reason: 'session_expired' }));
                                return null;
                            }
                            showFatalError('Sessão expirada. Feche e reabra.');
                            return null;
                        }
                        return res.json();
                    }).then(data => {
