...
if(chText){const chVal=parseInt(chText);const chSafe=isNaN(chVal)?50:chVal;const chCls=chSafe>=65?'prev-hit':chSafe>=40?'prev-mid':'prev-miss';const chLabel=chSafe>=65?'\u{1F3AF} Prov\u00e1vel':chSafe>=40?'\u26A0\uFE0F Arriscado':'\u274C Dif\u00edcil';previewHtml=`<div class="target-preview"><span class="${chCls}">${chLabel} \u00b7 ${chText}</span>${dmgText ? `\u00b7 ${escHtml(dmgText)}` : ''}</div>`;}}
