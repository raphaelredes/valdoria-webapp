...
if(e.se&&e.se.length>0){detailsHtml+='<div class="status-pills">'+e.se.map(s=>`<span class="status-pill${STATUS_BUFFS.has(s) ? ' buff status-buff' : ' status-debuff'}">${STATUS_ICONS[s] || ''} ${STATUS_PT[s] || s}</span>`).join('')+'</div>';}}
const isDead=e.hp<=0;const activeClass=isActiveTurn?' active-turn':'';const deadClass=isDead?' dead':'';const hpStateClass=pct>0.75?'':pct>0.50?' hp-wounded':pct>0.25?' hp-bloodied':pct>0?' hp-critical':'' /* noqa: preflight */;const dataAttr=type==='enemy'?` data-enemy-idx="${idx}"`:'';let posBadge='';if(type==='enemy'&&_currentPositions){const pPos=_currentPositions['player'];const ePos=_currentPositions[`enemy_${idx}`];if(pPos&&ePos){const dx=(pPos.x||0)-(ePos.x||0),dy=(pPos.y||0)-(ePos.y||0);const near=Math.sqrt(dx*dx+dy*dy)<=1.5;posBadge=`<span class="pos-badge ${near ? 'near' : 'far'}">${near ? '⚔ Perto' : '🏹 Longe'}</span>`;}}
...
function bindFeedDetail(){document.querySelectorAll('.feed-entry.has-detail').forEach(function(el){el.addEventListener('click',function(ev){ev.stopPropagation();var existing=el.querySelector('.feed-detail-expanded');if(existing){existing.remove();var arrow=el.querySelector('.feed-expand-icon');if(arrow)arrow.textContent='▸';return;}
...
function bindActions(state){document.querySelectorAll('.action-btn').forEach(btn=>{btn.addEventListener('click',()=>{hapticSelect();if(!_audioUnlocked){_audioUnlocked=true;if(_audioCtx&&_audioCtx.state==='suspended')_audioCtx.resume().catch(()=>{});}
const action=btn.dataset.action;if(btn.classList.contains('disabled')){btn.classList.add('disabled-shake');setTimeout(()=>btn.classList.remove('disabled-shake'),400);if(action==='skill')showCombatToast('Sem recurso suficiente para habilidades');else if(action==='items')showCombatToast('Nenhum item utilizável em combate');return;}
const enemies=state.e||[];const skills=state.acts?.skills||[];if(action==='attack'){if(enemies.length>1){showTargetPicker(enemies,'attack');}else{sendAction({type:'attack',target:0});}}else if(action==='skill'){if(skills.length===1){if(enemies.length>1){showTargetPicker(enemies,'skill',skills[0].id);}else{sendAction({type:'skill',skill_id:skills[0].id,target:0});}}else if(skills.length>1){showSkillPicker(skills,enemies,'skill');}}else if(action==='flee'){sendAction({type:'flee'});}else if(action==='pass'){sendAction({type:'pass'});}else if(action==='items'){const items=state.acts?.item_list||[];if(items.length>0){showItemPicker(items,enemies,state.a||[]);}}else if(action==='initiative'){sendAction({type:'initiative'});}else if(action==='proceed'){sendAction({type:'proceed'});}else if(action==='restore'){sendAction({type:'restore'});}
...
var _actionSent=false;var _actionSentTimer=null;async function sendAction(actionData){if(_actionSent||_cinematicInProgress)return;haptic('medium');_actionSent=true;clearTimeout(_actionSentTimer);_actionSentTimer=setTimeout(()=>{if(_actionSent){console.warn('[COMBAT] Action safety timeout — force reset');_actionSent=false;_showActionLoading(false);document.querySelectorAll('.action-btn').forEach(b=>b.classList.remove('disabled'));startPolling();}},30000);_lastAnimatedRoll=null;_initDiceAnimated=false;document.querySelectorAll('.action-btn').forEach(b=>b.classList.add('disabled'));_showActionLoading(true);stopTimer();stopPolling();if(isApiMode&&api){try{const result=await api.sendAction(actionData);clearTimeout(_actionSentTimer);_showActionLoading(false);_actionSent=false;if(!result){showError('Sem resposta do servidor. Verifique sua conex\u00e3o.');return;}
...
if(!item)return;haptic('light');const skillId=item.dataset.skillId;const tg=item.dataset.tg||'single';overlay.classList.remove('active');if(tg==='all'||tg==='self'){_overlayOpen=false;sendAction({type:actionType,skill_id:skillId,target:0});}else if(enemies.length>1){showTargetPicker(enemies,actionType,skillId);}else{_overlayOpen=false;sendAction({type:actionType,skill_id:skillId,target:0});}};overlay.onclick=(e)=>{if(e.target===overlay)_closeOverlay(overlay);};}
function _closeOverlay(overlay){overlay.classList.remove('active');_overlayOpen=false;}
function showTargetPicker(enemies,actionType,skillId){const panel=document.getElementById('targetPanel');const overlay=document.getElementById('targetOverlay');_overlayOpen=true;let html='<div class="skill-panel-title">Escolher Alvo</div>';enemies.forEach((e,i)=>{const pct=e.mhp>0?Math.round((e.hp/e.mhp)*100):0;let previewHtml='';const acts=currentState&&currentState.acts;if(acts){let chText='',dmgText='';if(actionType==='attack'){chText=`${acts.hit || '?'}%`;dmgText=currentState.p?currentState.p.dmg:'';}else if((actionType==='skill'||actionType==='bonus_use')&&skillId&&acts.skills){const sk=acts.skills.find(s=>s.id===skillId);if(sk){chText=`${sk.ch}%`;dmgText=sk.eff||'';}}
if(chText){const chVal=parseInt(chText);const chCls=chVal>=65?'prev-hit':chVal>=40?'prev-mid':'prev-miss';const chLabel=chVal>=65?'\u{1F3AF} Prov\u00e1vel':chVal>=40?'\u26A0\uFE0F Arriscado':'\u274C Dif\u00edcil';previewHtml=`<div class="target-preview"><span class="${chCls}">${chLabel} \u00b7 ${chText}</span>${dmgText ? `\u00b7 ${escHtml(dmgText)}` : ''}</div>`;}}
const hpColor=pct>50?'var(--v-hp-high, #5a8a3c)':pct>25?'var(--v-hp-mid, #b88a2a)':'var(--v-hp-low, #a63a2a)';html+=`<div class="target-item" data-target="${i}">
...
if(!item||_actionSent)return;haptic('medium');const target=parseInt(item.dataset.target);overlay.classList.remove('active');_overlayOpen=false;if(actionType==='skill'||actionType==='bonus_use'){sendAction({type:actionType,skill_id:skillId,target:target});}else{sendAction({type:'attack',target:target});}};overlay.onclick=(e)=>{if(e.target===overlay)_closeOverlay(overlay);};}
