...
const payload={action:'combat_action',token:token,...actionData,};tg.sendData(JSON.stringify(payload)); /* pflt-suppress: fetch via api.sendAction() above */setTimeout(()=>{try{tg.close();}catch(e){console.warn('[COMBAT] tg.close() failed',e);}},1000);}}
...
if(chText){const chVal=parseInt(chText, 10);let chCls;if(isNaN(chVal)){chCls='prev-mid';}else if(chVal>=65){chCls='prev-hit';}else if(chVal>=40){chCls='prev-mid';}else{chCls='prev-miss';};const chLabel=isNaN(chVal)?'❓ Incerto':chVal>=65?'\u{1F3AF} Prov\u00e1vel':chVal>=40?'\u26A0\uFE0F Arriscado':'\u274C Dif\u00edcil';previewHtml=`<div class="target-preview"><span class="${chCls}">${chLabel} \u00b7 ${chText}</span>${dmgText ? `\u00b7 ${escHtml(dmgText)}` : ''}</div>`;}}
