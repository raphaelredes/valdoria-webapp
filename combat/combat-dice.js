function _dice3dTextFallback(value,label,onDone,delayMs){
delayMs=delayMs||800;
var fb=document.createElement('div');
fb.className='dice-text-fallback';
fb.innerHTML='<div class="dice-fb-icon">\uD83C\uDFB2</div><div class="dice-fb-value">'+(value!=null?value:'?')+'</div>'+(label?'<div class="dice-fb-label">'+label+'</div>':'');
...
if(_pendingVfx){setTimeout(_pendingVfx,50);_pendingVfx=null;}};if(hasAdvDis){try{canvas.classList.add('multi');_dmgDice3d=new Dice3D(canvas,{size:typeof ValdoriaDice!=='undefined'?ValdoriaDice.D20_ADV_SIZE:260,dieType:'d20',duration:typeof ValdoriaDice!=='undefined'?ValdoriaDice.TIMING.D20_ROLL_MS:1200,particlesContainer:particles});const configs=lr.d20s.map(v=>({value:Math.min(v,20),type:'d20'}));_dmgDice3d.rollMultiple(configs,()=>{if(_done)return;const chosen=lr.r;const d1=lr.d20s[0],d2=lr.d20s[1];const advLabel=lr.adv?'Vantagem':'Desvantagem';if(label3d){const s1=d1===chosen?'font-weight:bold;color:var(--v-gold)':'opacity:0.4;text-decoration:line-through';const s2=d2===chosen?'font-weight:bold;color:var(--v-gold)':'opacity:0.4;text-decoration:line-through';label3d.innerHTML=`<span style="font-size:0.8em">${advLabel}</span><br><span style="${s1}">${d1}</span> / <span style="${s2}">${d2}</span> → <b>${chosen}</b>`;label3d.className='dmg-dice3d-label hit';}
setTimeout(()=>{if(_done)return;canvas.classList.remove('multi');_processAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,()=>{_done=true;},(vfx)=>{_pendingVfx=vfx;});},ValdoriaMotion.duration(1200,600));});}catch(e){console.warn('[COMBAT] Adv/Dis Dice3D failed, falling back:',e);canvas.classList.remove('multi');_initDiceAttackOverlaySingle(lr,overlay,canvas,particles,label3d,skipBtn,finishAll,(vfx)=>{_pendingVfx=vfx;});}
return;}
_initDiceAttackOverlaySingle(lr,overlay,canvas,particles,label3d,skipBtn,finishAll,(vfx)=>{_pendingVfx=vfx;});}
function _initDiceAttackOverlaySingle(lr,overlay,canvas,particles,label3d,skipBtn,finishAll,setPendingVfx){let _done=false;const markDone=()=>{_done=true;};try{_dmgDice3d=new Dice3D(canvas,{size:typeof ValdoriaDice!=='undefined'?ValdoriaDice.D20_SIZE:220,dieType:'d20',duration:typeof ValdoriaDice!=='undefined'?ValdoriaDice.TIMING.D20_ROLL_MS:1200,particlesContainer:particles});const faceValue=Math.min(lr.r,20);_dmgDice3d.roll(faceValue,()=>{if(_done)return;_processAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx);});}catch(e){console.warn('[COMBAT] Attack overlay Dice3D failed:',e);overlay.style.display='none';_dice3dTextFallback(lr.r,'d20',function(){finishAll();});}}
function _processAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx){const isCrit=lr.crit||lr.r===20;const isFail=lr.r===1;const isMiss=lr.miss||lr.d<=0;/* Slow-motion anticipation for critical hits — 450ms dramatic pause */if(isCrit&&label3d&&overlay){label3d.textContent='';overlay.classList.add('crit-anticipation');console.info('[COMBAT:DICE] crit_anticipation nat20=%s',lr.r===20);setTimeout(function(){overlay.classList.remove('crit-anticipation');_revealCritResult(lr,label3d);_finishAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx,isCrit,isFail,isMiss);},ValdoriaMotion.duration(450,200));return;}if(label3d){if(isCrit){_revealCritResult(lr,label3d);}else if(isFail){label3d.innerHTML='<span style="font-size:1.3em">&#x1F480;</span> FALHA CRITICA!' /* noqa: safe — static content */;label3d.className='dmg-dice3d-label miss';vHaptic.error();showNarration(_pick(_NARR_NAT1),'nat1');_hitStreak=0;}else if(isMiss){const total=lr.total||lr.r;var _bkMiss=_formatBreakdown(lr);label3d.innerHTML='&#x1F6E1;&#xFE0F; Errou!'+(_bkMiss?'<br><span class="breakdown-line">'+_bkMiss+'</span>':' <span style="font-size:0.7em">('+total+' vs CA '+lr.ac+')</span>') /* noqa: safe — server data */;label3d.className='dmg-dice3d-label miss';vHaptic.burst('miss');sfxMiss();if(typeof announce==='function')announce('Errou! d20: '+(lr.total||lr.r)+' vs CA '+lr.ac);showNarration(_pick(_NARR_MISS),'miss');_hitStreak=0;}else{const total=lr.total||lr.r;const hitLabel=lr.t==='skill'?'Habilidade!':'Acertou!';var _bkHit=_formatBreakdown(lr);label3d.innerHTML='&#x2694;&#xFE0F; '+hitLabel+(_bkHit?'<br><span class="breakdown-line">'+_bkHit+'</span>':' <span style="font-size:0.7em">('+total+' vs CA '+lr.ac+')</span>') /* noqa: safe — server data */;label3d.className='dmg-dice3d-label hit';vHaptic.medium();sfxHit();if(typeof announce==='function')announce(hitLabel+' d20: '+(lr.total||lr.r)+' vs CA '+lr.ac);_hitStreak++;if(_hitStreak>=2)_showComboCounter(_hitStreak);}}_finishAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx,isCrit,isFail,isMiss);}
function _revealCritResult(lr,label3d){var _bkLine=_formatBreakdown(lr);label3d.innerHTML='<span style="font-size:1.3em">&#x1F31F;</span> CRITICO!'+(_bkLine?'<br><span class="breakdown-line">'+_bkLine+'</span>':' <span style="font-size:0.7em">('+lr.r+' vs CA '+lr.ac+')</span>') /* noqa: safe — server data */;label3d.className='dmg-dice3d-label crit';vHaptic.burst('crit');sfxCrit();if(typeof announce==='function')announce('Critico! d20: '+lr.r+' vs CA '+lr.ac);showNarration(_pick(_NARR_CRIT),'crit');_hitStreak++;if(_hitStreak>=2)_showComboCounter(_hitStreak);}
function _finishAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx,isCrit,isFail,isMiss){/* Background color shift by result type */if(overlay){overlay.classList.remove('result-hit','result-miss','result-crit');if(isCrit)overlay.classList.add('result-crit');else if(isMiss||isFail)overlay.classList.add('result-miss');else overlay.classList.add('result-hit');}setPendingVfx(()=>{if(isCrit)_shakeApp('heavy');if(isMiss){const dodgeTarget=document.querySelector('.cell[data-unit-id^="enemy_"]');if(dodgeTarget){dodgeTarget.classList.add('dodge-flash');setTimeout(()=>dodgeTarget.classList.remove('dodge-flash'),ValdoriaMotion.duration(400,0));}
_showMissFloat('.cell[data-unit-id^="enemy_"]');if(window._combatVfx){const _pEl=document.querySelector('.cell[data-unit-id="player"]');if(_pEl&&dodgeTarget)window._combatVfx.miss(_pEl,dodgeTarget,lr.dt||'slashing');}}else if(lr.d>0&&window._combatVfx){const _pEl=document.querySelector('.cell[data-unit-id="player"]');const _eEl=document.querySelector('.cell[data-unit-id^="enemy_"]');if(_pEl&&_eEl)window._combatVfx.projectile(_pEl,_eEl,lr.dt||'slashing',{crit:!!isCrit});}});if(isMiss||lr.d<=0){setTimeout(()=>{if(skipBtn){skipBtn.classList.add('visible');skipBtn.onclick=finishAll;}},500);setTimeout(finishAll,ValdoriaMotion.duration(2400,1200));return;}
...
vHaptic.tap();sfxDiceRoll();let _done=false;let _pendingVfx=null;const finishAll=()=>{if(_done)return;_done=true;_diceAnimating=false;if(skipBtn){skipBtn.classList.remove('visible');skipBtn.onclick=null;}
overlay.style.display='none';canvas.classList.remove('multi','multi-3','multi-4','multi-5');if(_dmgDice3d){_dmgDice3d.dispose();_dmgDice3d=null;}
if(_pendingVfx){setTimeout(_pendingVfx,50);_pendingVfx=null;}};try{_dmgDice3d=new Dice3D(canvas,{size:typeof ValdoriaDice!=='undefined'?ValdoriaDice.D20_SIZE:220,dieType:'d20',duration:typeof ValdoriaDice!=='undefined'?ValdoriaDice.TIMING.D20_ROLL_MS:1200,particlesContainer:particles});const faceValue=Math.min(lr.r,20);_dmgDice3d.roll(faceValue,()=>{if(_done)return;if(label3d){if(lr.saved&&!lr.half){label3d.innerHTML='&#x1F6E1;&#xFE0F; Resistiu! <span style="font-size:0.7em">('+lr.r+' vs CD '+lr.dc+')</span>';label3d.className='dmg-dice3d-label miss';vHaptic.burst('miss');sfxMiss();}else if(lr.saved&&lr.half){label3d.innerHTML='&#x26A0;&#xFE0F; Parcial <span style="font-size:0.7em">('+lr.r+' vs CD '+lr.dc+')</span>';label3d.className='dmg-dice3d-label miss';vHaptic.medium();}else{label3d.innerHTML='&#x274C; Falhou! <span style="font-size:0.7em">('+lr.r+' vs CD '+lr.dc+')</span>';label3d.className='dmg-dice3d-label hit';vHaptic.medium();sfxHit();}}
_pendingVfx=()=>{if(lr.saved&&!lr.half){if(window._combatVfx){const _pEl=document.querySelector('.cell[data-unit-id="player"]');const _eEl=document.querySelector('.cell[data-unit-id^="enemy_"]');if(_pEl&&_eEl)window._combatVfx.miss(_pEl,_eEl,lr.dt||'fire');}}else if(lr.d>0&&window._combatVfx){const _pEl=document.querySelector('.cell[data-unit-id="player"]');const _eEl=document.querySelector('.cell[data-unit-id^="enemy_"]');if(_pEl&&_eEl)window._combatVfx.projectile(_pEl,_eEl,lr.dt||'fire',{crit:false});}};if(lr.saved&&!lr.half){setTimeout(()=>{if(!_done&&skipBtn){skipBtn.classList.add('visible');skipBtn.onclick=finishAll;}},500);setTimeout(finishAll,ValdoriaMotion.duration(2000,1000));return;}
...
overlay.style.display='none';canvas.classList.remove('multi','multi-3','multi-4','multi-5');if(_dmgDice3d){_dmgDice3d.dispose();_dmgDice3d=null;}
if(window._combatVfx){setTimeout(()=>{const _pEl=document.querySelector('.cell[data-unit-id="player"]');const _eEl=document.querySelector('.cell[data-unit-id^="enemy_"]');if(_pEl&&_eEl)window._combatVfx.projectile(_pEl,_eEl,lr.dt||'force',{crit:false});},50);}};setTimeout(()=>{if(_done)return;_initDamagePhase(lr,overlay,canvas,particles,label3d,skipBtn,finishAll,()=>{_done=true;});},ValdoriaMotion.duration(300,150));}}
...
overlay.style.display='none';canvas.classList.remove('multi','multi-3','multi-4','multi-5');if(_dmgDice3d){_dmgDice3d.dispose();_dmgDice3d=null;}
vHaptic.heavy();const app=document.getElementById('app');if(app){var _hitCls=lr.crit?'hit-stop-crit':'hit-stop';var _hitDur=lr.crit?180:120;app.classList.add(_hitCls);setTimeout(()=>app.classList.remove(_hitCls),ValdoriaMotion.duration(_hitDur,0));}
if(typeof sfxDamageType==='function')sfxDamageType(lr.dt||'slashing');const enemies=document.querySelectorAll('.cell[data-unit-id^="enemy_"]');const isAoe=lr.t==='aoe';const hitCount=isAoe?Math.min(lr.hits||enemies.length,enemies.length):1;for(let i=0;i<hitCount&&i<enemies.length;i++){const delay=isAoe?i*100:0;setTimeout(()=>{const flashCls=_dmgFlashClass(lr.dt);enemies[i].classList.add(flashCls);setTimeout(()=>enemies[i].classList.remove(flashCls),ValdoriaMotion.duration(400,0));const hpBar=enemies[i].querySelector('.hp-mini');if(hpBar){hpBar.classList.add('dmg-taken');setTimeout(()=>hpBar.classList.remove('dmg-taken'),ValdoriaMotion.duration(500,200));}
if(window._combatVfx){window._combatVfx.impact(enemies[i],lr.dt||'slashing',{crit:!!lr.crit});}},delay);}
...
if(_pendingDsVfx){setTimeout(_pendingDsVfx,50);_pendingDsVfx=null;}};try{_dmgDice3d=new Dice3D(canvas,{size:typeof ValdoriaDice!=='undefined'?ValdoriaDice.D20_SIZE:220,dieType:'d20',duration:typeof ValdoriaDice!=='undefined'?ValdoriaDice.TIMING.DEATH_SAVE_MS:1800,particlesContainer:particles});const faceValue=Math.min(lr.r,20);_dmgDice3d.roll(faceValue,()=>{if(label3d){const isCrit=lr.crit;const isCritFail=lr.crit_fail;const isSuccess=lr.success;if(isCrit){label3d.textContent='🌟 NAT 20! Acordou!';label3d.className='dmg-dice3d-label crit';vHaptic.burst('crit');sfxCrit();}else if(isCritFail){label3d.textContent='💀 NAT 1! Falha Dupla!';label3d.className='dmg-dice3d-label miss';vHaptic.error();}else if(isSuccess){label3d.textContent=`✅ ${lr.r} — Sucesso!`;label3d.className='dmg-dice3d-label hit';vHaptic.medium();}else{label3d.textContent=`❌ ${lr.r} — Falha!`;label3d.className='dmg-dice3d-label miss';vHaptic.tap();}
_pendingDsVfx=()=>{if(isCrit){if(window._combatVfx){const pEl=document.querySelector('.cell[data-unit-id="player"]');if(pEl)window._combatVfx.buff(pEl);window._combatVfx.flash('rgba(255,215,0,0.4)',500);}
...
overlay.style.display='none';canvas.classList.remove('multi','multi-3','multi-4','multi-5');if(_dmgDice3d){_dmgDice3d.dispose();_dmgDice3d=null;}
if(isHeal){const _pEl=document.querySelector('.cell[data-unit-id="player"]');if(window._combatVfx&&_pEl)window._combatVfx.heal(_pEl);showNarration(_pick(_NARR_HEAL),'heal');if(lr.d>0)_showHealFloat(lr.d,'.cell[data-unit-id="player"]');_showHealFlash();const hpTrack=_pEl?_pEl.querySelector('.bar-track'):null;if(hpTrack){hpTrack.classList.add('heal-glow');setTimeout(()=>hpTrack.classList.remove('heal-glow'),ValdoriaMotion.duration(600,0));}}else{const _pEl=document.querySelector('.cell[data-unit-id="player"]');if(window._combatVfx&&_pEl)window._combatVfx.buff(_pEl);showNarration(_pick(_NARR_BUFF).replace('{name}',lr.sn||'efeito'),'');vHaptic.medium();}};try{const canvasSize=typeof ValdoriaDice!=='undefined'?(ValdoriaDice.CANVAS_SIZES[parsed.count]||180):(parsed.count>=2?200:180);_dmgDice3d=new Dice3D(canvas,{size:canvasSize,dieType:parsed.type,duration:1500,particlesContainer:particles});let individualResults;if(lr.dr&&lr.dr.length>=parsed.count)individualResults=lr.dr.slice(0,parsed.count);else individualResults=_distributeTotal(Math.max(1,lr.d-parsed.modifier),parsed.count,parsed.sides);if(parsed.count>=2){const configs=individualResults.map(v=>({value:v}));_dmgDice3d.rollMultiple(configs,()=>{if(label3d){const modText=parsed.modifier?` + ${parsed.modifier}`:'';label3d.textContent=individualResults.join(' + ')+modText;label3d.className='dmg-dice3d-label hit';}
setTimeout(()=>{if(_efDone)return;if(label3d){label3d.textContent='';label3d.className='dmg-dice3d-label rolling';}
_dmgDice3d.fusionTo(lr.d,()=>{if(label3d){label3d.textContent=isHeal?`+${lr.d} cura`:`${lr.d} efeito`;label3d.className='dmg-dice3d-label hit';}
setTimeout(()=>{if(!_efDone&&skipBtn){skipBtn.classList.add('visible');skipBtn.onclick=finishEf;}},500);setTimeout(finishEf,ValdoriaMotion.duration(2000,1000));});},typeof ValdoriaDice!=='undefined'?ValdoriaDice.TIMING.FUSION_HOLD:600);});}else{const fv=individualResults[0]||1;_dmgDice3d.roll(fv,()=>{if(label3d){label3d.textContent=isHeal?`+${lr.d} cura`:`${lr.d} efeito`;label3d.className='dmg-dice3d-label hit';}
setTimeout(()=>{if(!_efDone&&skipBtn){skipBtn.classList.add('visible');skipBtn.onclick=finishEf;}},500);setTimeout(finishEf,ValdoriaMotion.duration(2000,1000));});}}catch(e){console.warn('[COMBAT] Effect Dice3D failed:',e);overlay.style.display='none';canvas.classList.remove('multi','multi-3','multi-4','multi-5');_dice3dTextFallback(lr.d,lr.t==='heal'?'+'+lr.d+' cura':lr.d+' efeito',function(){finishEf();});}}}
function _parseDiceFormula(formula){if(typeof ValdoriaDice!=='undefined')return ValdoriaDice.parseDiceFormula(formula);if(!formula)return{count:1,type:'d6',sides:6,modifier:0};const m=formula.match(/(\d*)d(\d+)/i);if(!m)return{count:1,type:'d6',sides:6,modifier:0};const count=parseInt(m[1],10)||1;const sides=parseInt(m[2],10);const valid=[4,6,8,10,12,20];const type=valid.includes(sides)?`d${sides}`:'d6';const actualSides=valid.includes(sides)?sides:6;const rest=formula.slice(m.index+m[0].length);const modMatch=rest.match(/([+-]\d+)/);const modifier=modMatch?parseInt(modMatch[1],10):0;return{count:Math.min(count,5),type,sides:actualSides,modifier};}
function _parseDieType(formula){return _parseDiceFormula(formula).type;}
function _distributeTotal(diceTotal,count,sides){if(typeof ValdoriaDice!=='undefined')return ValdoriaDice.distributeTotal(diceTotal,count,sides);const results=[];let remaining=Math.max(count,Math.min(diceTotal,count*sides));for(let i=0;i<count-1;i++){const minNeeded=count-i-1;const maxAllowed=remaining-minNeeded;const value=Math.min(sides,Math.max(1,Math.floor(Math.random()*Math.min(sides,maxAllowed))+1));results.push(value);remaining-=value;}
results.push(Math.min(sides,Math.max(1,remaining)));return results;}