...
function _finishAttackResult(lr,label3d,overlay,canvas,particles,skipBtn,finishAll,markDone,setPendingVfx,isCrit,isFail,isMiss){/* Background color shift by result type */if(overlay){overlay.classList.remove('result-hit','result-miss','result-crit');if(isCrit)overlay.classList.add('result-crit');else if(isMiss||isFail)overlay.classList.add('result-miss');else overlay.classList.add('result-hit');}setPendingVfx(()=>{if(isCrit)_shakeApp('heavy');if(isMiss){const dodgeTarget=document.querySelector('.entity.enemy');if(dodgeTarget){dodgeTarget.classList.add('dodge-flash');setTimeout(()=>dodgeTarget.classList.remove('dodge-flash'),ValdoriaMotion.duration(400,0));}
...
if(typeof sfxDamageType==='function')sfxDamageType(lr.dt||'slashing');const enemies=document.querySelectorAll('.entity.enemy');const isAoe=lr.t==='aoe';const hitCount=isAoe?Math.min(lr.hits||enemies.length,enemies.length):1;for(let i=0;i<hitCount&&i<enemies.length;i++){const delay=isAoe?i*100:0;setTimeout(()=>{const flashCls=_dmgFlashClass(lr.dt);enemies[i].classList.add(flashCls);setTimeout(()=>enemies[i].classList.remove(flashCls),ValdoriaMotion.duration(400,0));const hpBar=enemies[i].querySelector('.hp-mini');if(hpBar){hpBar.classList.add('dmg-taken');setTimeout(()=>hpBar.classList.remove('dmg-taken'),ValdoriaMotion.duration(500,200));}
...
_pendingDsVfx=()=>{if(isCrit){if(window._combatVfx){const pEl=document.querySelector('.entity.player');if(pEl)window._combatVfx.buff(pEl);window._combatVfx.flash('rgba(255,215,0,0.4)',500);}
