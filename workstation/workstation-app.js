var _spaP=window.__spaRouteParams||{};var tg=window.Telegram?.WebApp;var _vBg=getComputedStyle(document.documentElement).getPropertyValue('--v-bg').trim()||'#2a2420';var _wsParams=new URLSearchParams(window.location.search);var _wsApi=_wsParams.get('api')||'';var _wsUid=_wsParams.get('uid')||'';var _wsToken=_wsParams.get('token')||'';var _wsReturn=_wsParams.get('return')||'game';if(tg){tg.ready();tg.expand();tg.setHeaderColor(_vBg);tg.setBackgroundColor(_vBg);if(tg.BackButton){tg.BackButton.show();tg.BackButton.onClick(()=>{_navigateBack();});}}
if(window.ValdoriaErrors){ValdoriaErrors.init({appName:'WORKSTATION',apiBase:_wsApi,token:_wsToken,uid:_wsUid});}
async function _navigateBack(){if(_wsApi&&_wsToken){const h={'Content-Type':'application/json','Authorization':'Bearer '+_wsToken};if(window.Telegram?.WebApp?.initData)h['X-Telegram-Init-Data']=Telegram.WebApp.initData;h['X-Idempotency-Key']=crypto.randomUUID();try{const r=await fetchT(_wsApi+'/api/webapp/transition',{method:'POST',headers:h,body:JSON.stringify({from:'workstation',to:_wsReturn,user_id:parseInt(_wsUid,10),payload:{}})});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();if(d.url){window.__valdoria_transitioning=true;valdoriaSpaNav(d.url);return;}}catch(e){console.error('[WORKSTATION] transition error:',e);if(typeof vToast==='function')vToast('Erro ao voltar. Tentando fechar...','error',2500);}}
window.__valdoria_transitioning=true;try{if(tg)tg.close();}catch(e){console.warn('[WORKSTATION] tg.close:',e);}}
var PAYLOAD=null;var selectedRecipe=null;var activeTab=null;var RARITY_LABELS={common:'Comum',uncommon:'Incomum',rare:'Raro',very_rare:'Muito Raro',legendary:'Lendário',};
...
function updateHeader(){const stats=document.getElementById('headerStats');const matCount=Object.values(PAYLOAD.materials).reduce((a,b)=>a+b,0);stats.innerHTML='<span class="vi vi-coin sm"></span> '+(Number(PAYLOAD.gold)||0)+' <span class="vi vi-coin sm"></span> \u00b7 \ud83d\udce6 '+matCount+' materiais \u00b7 Lv.'+(Number(PAYLOAD.level)||1);}
...
if(tg){window.__valdoria_transitioning=true;tg.sendData(JSON.stringify(payload));setTimeout(()=>{try{tg.close();}catch(e){console.warn('[WORKSTATION] tg.close:',e);}},300);}}
