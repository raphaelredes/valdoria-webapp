...
function _renderRecruitList() {
    ...
    for (var j = 0; j < recruits.length; j++) {
        ...
        html += '</div>';
        html += '<div class="guild-card-sub">' + _esc(r.class_name || '') + ' Nv.' + (r.level || 1) + ' &middot; ' + (r.cost || 0) + ' GP';
...
window.showRecruitDetail = function(npcId) {
    ...
    html += '<div style="font-size:var(--v-font-body);color:var(--v-gold)">' + (r.cost || 0) + ' GP</div>';
    ...
    html += '<button class="guild-action-btn primary"' + disabled + ' onclick="_hireRecruit(\'' + _esc(r.id) + '\')">&#x1F4B0; Contratar (' + (r.cost || 0) + ' GP)</button>';
