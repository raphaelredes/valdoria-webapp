...
function _renderTraining() {
    ...
    if (d.active) {
        ...
        html += '<button class="guild-action-btn primary"' + (!canPay ? ' disabled title="Ouro insuficiente"' : '') + ' onclick="_advanceTraining()">&#x23E9; Avançar (' + (d.cost || 25) + ' <span class="vi vi-coin sm"></span>)</button>';
    ...
    if (!d.active) {
        ...
        for (var c = 0; c < cats.length; c++) {
            ...
            for (var j = 0; j < available.length; j++) {
                ...
                html += '<div class="guild-training-meta">' + (d.sessions_total || '?') + ' sessoes &middot; ' + (d.cost || 25) + ' <span class="vi vi-coin sm"></span>/sessao</div>';
                html += '</div>';
            }
        }
