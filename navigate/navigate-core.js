...
function finishNavigation(type, target) {
        console.log('[NAVIGATE][DEBUG] finishNavigation called:', { type, target, _navSent });
    ...
    const apiUrl = S.api ? `${S.api}/api/navigate/action` : null;

    if (apiUrl) {
                console.log('[NAVIGATE][DEBUG] sending via fetch to', apiUrl, payload);
                fetch(apiUrl, {
                                method: 'POST',
                ...
                .then(d => console.log('[NAVIGATE][DEBUG] fetch response:', d))
                .catch(e => console.error('[NAVIGATE][DEBUG] fetch error:', e))
                .finally(() => {
                                setTimeout(() => { try { tg?.close(); } catch(e) {} }, 300);
                });
    } else {
                // Fallback: sendData (only works with keyboard buttons, may fail silently)
            console.warn('[NAVIGATE][DEBUG] No api param - falling back to sendData');
                try {
                                if (tg?.sendData) tg.sendData(JSON.stringify(payload));
                } catch(e) {
                                console.error('[NAVIGATE][DEBUG] sendData error:', e);
                }
