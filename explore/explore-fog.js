...
if(typeof S!=='undefined'&&S.torches&&S.torches.length>0){var _torchTier=_fogRD;var _tNow=performance.now();for(var ti=0;ti<S.torches.length;ti++){var torch=S.torches[ti];var tKey=torch.col+','+torch.row;if(fogState[tKey]!=='visible'&&fogState[tKey]!=='dim')continue;var tCenter=hexToScreen(torch.col,torch.row);/* Flicker: ±5% alpha variation via sin wave per torch */var _flicker=1.0+0.05*Math.sin(_tNow*0.003+ti*2.1);var _tRBase=Math.max(torch.radius||2,3.5);var tRad=HEX_W*(_tRBase*1.0+0.5)*_flicker;/* Phase 1: Full punch through fog */_fogCtx.globalCompositeOperation='destination-out';if(_torchTier>=1){var tGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,tRad);tGrad.addColorStop(0,'rgba(0,0,0,1)');tGrad.addColorStop(0.25,'rgba(0,0,0,0.9)');tGrad.addColorStop(0.5,'rgba(0,0,0,0.5)');tGrad.addColorStop(0.75,'rgba(0,0,0,0.15)');tGrad.addColorStop(1,'rgba(0,0,0,0)');_fogCtx.fillStyle=tGrad;}else{_fogCtx.fillStyle='rgba(0,0,0,0.8)';}_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,tRad,0,Math.PI*2);_fogCtx.fill();/* Phase 2: Strong warm amber glow */if(_torchTier>=1){_fogCtx.globalCompositeOperation='source-over';var warmRad=tRad*0.85;var warmGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,warmRad);var _fa=0.35*_flicker;warmGrad.addColorStop(0,'rgba(255,200,80,'+_fa.toFixed(3)+')');warmGrad.addColorStop(0.3,'rgba(240,160,50,'+(_fa*0.6).toFixed(3)+')');warmGrad.addColorStop(0.6,'rgba(200,120,30,'+(_fa*0.25).toFixed(3)+')');warmGrad.addColorStop(1,'rgba(160,90,20,0)');_fogCtx.fillStyle=warmGrad;_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,warmRad,0,Math.PI*2);_fogCtx.fill();}}}
