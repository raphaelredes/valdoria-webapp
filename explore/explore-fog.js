...
if(typeof S!=='undefined'&&S.torches&&S.torches.length>0){var _torchTier=_fogRD;var _tNow=performance.now();for(var ti=0;ti<S.torches.length;ti++){var torch=S.torches[ti];var tKey=torch.col+','+torch.row;if(fogState[tKey]!=='visible'&&fogState[tKey]!=='dim')continue;var tCenter=hexToScreen(torch.col,torch.row);/* Flicker: ±5% alpha variation via sin wave per torch */var _flicker=1.0+0.05*Math.sin(_tNow*0.003+ti*2.1);var tRad=HEX_W*((torch.radius||2.5)*0.75+0.3)*_flicker;/* Phase 1: Punch hole in fog (destination-out) */_fogCtx.globalCompositeOperation='destination-out';if(_torchTier>=1){var tGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,tRad);tGrad.addColorStop(0,'rgba(0,0,0,0.85)');tGrad.addColorStop(0.4,'rgba(0,0,0,0.5)');tGrad.addColorStop(0.75,'rgba(0,0,0,0.15)');tGrad.addColorStop(1,'rgba(0,0,0,0)');_fogCtx.fillStyle=tGrad;}else{_fogCtx.fillStyle='rgba(0,0,0,0.5)';}_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,tRad,0,Math.PI*2);_fogCtx.fill();/* Phase 2: Warm amber tint overlay (source-over) */if(_torchTier>=1){_fogCtx.globalCompositeOperation='source-over';var warmRad=tRad*0.7;var warmGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,warmRad);var _fa=Math.round(30*_flicker);warmGrad.addColorStop(0,'rgba(255,180,60,'+(_fa/255).toFixed(3)+')');warmGrad.addColorStop(0.6,'rgba(220,140,30,'+(_fa*0.4/255).toFixed(3)+')');warmGrad.addColorStop(1,'rgba(180,100,20,0)');_fogCtx.fillStyle=warmGrad;_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,warmRad,0,Math.PI*2);_fogCtx.fill();}}}
