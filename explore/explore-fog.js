...
for(var r=0;r<ROWS;r++){for(var c=0;c<COLS;c++){var key=c+','+r;var state=fogState[key];var dist=hexDist(c,r,pC,pR);var alpha;if(state==='visible'){if(dist===0)alpha=0;else if(dist===1)alpha=3;else if(dist===2)alpha=120;else if(dist<=_visR)alpha=210;else alpha=240;}else if(state==='dim'){alpha=235;}else{alpha=255;}
...
if(typeof S!=='undefined'&&S.torches&&S.torches.length>0){var _torchTier=_fogRD;var _tNow=performance.now();for(var ti=0;ti<S.torches.length;ti++){var torch=S.torches[ti];var tKey=torch.col+','+torch.row;if(fogState[tKey]!=='visible'&&fogState[tKey]!=='dim')continue;var tCenter=hexToScreen(torch.col,torch.row);/* Flicker: ±5% alpha variation via sin wave per torch */var _flicker=1.0+0.05*Math.sin(_tNow*0.003+ti*2.1);var tRad=HEX_W*((torch.radius||3)*0.9+0.4)*_flicker;/* Phase 1: Punch hole in fog — stronger alpha to cut through aggressive fog */_fogCtx.globalCompositeOperation='destination-out';if(_torchTier>=1){var tGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,tRad);tGrad.addColorStop(0,'rgba(0,0,0,0.95)');tGrad.addColorStop(0.3,'rgba(0,0,0,0.75)');tGrad.addColorStop(0.6,'rgba(0,0,0,0.4)');tGrad.addColorStop(0.85,'rgba(0,0,0,0.1)');tGrad.addColorStop(1,'rgba(0,0,0,0)');_fogCtx.fillStyle=tGrad;}else{_fogCtx.fillStyle='rgba(0,0,0,0.7)';}_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,tRad,0,Math.PI*2);_fogCtx.fill();/* Phase 2: Warm amber tint — more visible glow */if(_torchTier>=1){_fogCtx.globalCompositeOperation='source-over';var warmRad=tRad*0.8;var warmGrad=_fogCtx.createRadialGradient(tCenter.x,tCenter.y,0,tCenter.x,tCenter.y,warmRad);var _fa=Math.round(50*_flicker);warmGrad.addColorStop(0,'rgba(255,180,60,'+(_fa/255).toFixed(3)+')');warmGrad.addColorStop(0.4,'rgba(240,150,40,'+(_fa*0.5/255).toFixed(3)+')');warmGrad.addColorStop(0.7,'rgba(200,120,30,'+(_fa*0.2/255).toFixed(3)+')');warmGrad.addColorStop(1,'rgba(180,100,20,0)');_fogCtx.fillStyle=warmGrad;_fogCtx.beginPath();_fogCtx.arc(tCenter.x,tCenter.y,warmRad,0,Math.PI*2);_fogCtx.fill();}}}
