...
setTimeout(()=>scrollCanvasToPlayer(),100);const _lc=window._loadingCtrl;if(_lc&&!restored){_lc.setProgress(100);_lc.hideLoading(()=>{if(S.dmIntro)showDMIntro(S.dmIntro);if(!S.travelActivity&&typeof showActivitySelection==='function'){const actDelay=S.dmIntro?4000:800;setTimeout(()=>{if(!S.travelActivity&&!_tutorialActive)showActivitySelection();},actDelay);}
...
function getAbilityMod(statKey){const abilityKey=SKILL_TO_ABILITY[statKey]||statKey;const val=(S.charData&&S.charData[abilityKey])||10;return Math.floor((val-10)/2);}
...
if(steps>15){var extraSteps=steps-15;var dc=10+extraSteps;var conMod=getAbilityMod('cn');var rollResult=rollD20('normal');if(rollResult.roll+conMod<dc){if((S._fatigue||0)>0&&(S._fatigue||0)<2){S._fatigue=2;showTerrainToast('Fadiga severa! Descanse ou ganhar\u00e1 exaust\u00e3o.','damage');updateFatigueHUD();}else{addExhaustion(1,'Marcha for\u00e7ada');}}}
