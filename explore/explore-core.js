...
setTimeout(()=>scrollCanvasToPlayer(),100);/* Force-hide ALL loading overlays — map is ready, don't block with MIN_LOAD_MS */if(typeof forceHideLoading==='function')forceHideLoading();var _shellLd=document.getElementById('loading');if(_shellLd&&!_shellLd.classList.contains('hidden')){_shellLd.classList.add('hidden');_shellLd.style.display='none';console.info('[EXPLORE] shell loading force-hidden — map ready');}const _lc=window._loadingCtrl;if(_lc&&!restored){_lc.setProgress(100);_lc.hideLoading(()=>{if(S.dmIntro)showDMIntro(S.dmIntro);if(!S.travelActivity&&typeof showActivitySelection==='function'){const actDelay=S.dmIntro?4000:800;setTimeout(()=>{if(!S.travelActivity&&!_tutorialActive)showActivitySelection();},actDelay);}
