...
var _onVisHideSave=function(){if(document.visibilityState==='hidden'){flushSaveState();}};document.addEventListener('visibilitychange',_onVisHideSave);if(!window._spaCleanupFns)window._spaCleanupFns=[];window._spaCleanupFns.push(function(){window.removeEventListener('pagehide',_onPageHideWatchdog);document.removeEventListener('visibilitychange',_onVisHideSave);if(_overlayWatchdogTimer){clearInterval(_overlayWatchdogTimer);_overlayWatchdogTimer=null;}/* Remove overlays criados pelo explore que podem vazar em outras telas SPA */var _expLeakIds=['bark-bubble','terrain-toast-el','tutorial-overlay','tutorial-confirm','dm-overlay','encounter-overlay','combat-overlay','lowhp-overlay','camp-result-overlay','return-journey-overlay','exh-warn-overlay'];for(var _eli=0;_eli<_expLeakIds.length;_eli++){var _eov=document.getElementById(_expLeakIds[_eli]);if(_eov&&_eov.parentNode){_eov.parentNode.removeChild(_eov);console.log('[EXPLORE] SPA cleanup: removed',_expLeakIds[_eli]);}}var _expLeakSel=['.encounter-flash'];for(var _esi=0;_esi<_expLeakSel.length;_esi++){var _ens=document.querySelectorAll(_expLeakSel[_esi]);for(var _eni=0;_eni<_ens.length;_eni++){if(_ens[_eni].parentNode)_ens[_eni].parentNode.removeChild(_ens[_eni]);}}});
