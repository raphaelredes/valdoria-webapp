...
var _actionSent=false;var _actionSentTimer=null;var _actionAbort=null;async function sendAction(actionData){window._lastActionTarget=actionData.target||0;var _stateNow=(typeof currentState!=='undefined')?currentState:null;console.info('[COMBAT:ACTION] sendAction type=%s target=%s skill=%s item=%s tc=%s activeType=%s enemyHP=%s playerHP=%s',actionData.type,actionData.target,actionData.skill_id||'-',actionData.item_key||'-',(_stateNow&&_stateNow.tc)||'?',(_stateNow&&_stateNow.active_turn&&_stateNow.active_turn.t)||'?',(_stateNow&&_stateNow.e&&_stateNow.e[actionData.target||0]&&_stateNow.e[actionData.target||0].hp)||'?',(_stateNow&&_stateNow.p&&_stateNow.p.hp)||'?');if(_actionSent||_cinematicInProgress){console.warn('[COMBAT:ACTION] BLOCKED actionSent=%s cinematic=%s',_actionSent,_cinematicInProgress);return;}if(typeof _clearReactionAutoTimers==='function')_clearReactionAutoTimers();vHaptic.medium();_actionSent=true;clearTimeout(_actionSentTimer);_actionSentTimer=setTimeout(()=>{if(_actionSent){console.warn('[COMBAT] Action safety timeout — force reset; aborting in-flight fetch');if(_actionAbort){_actionAbort.abort();_actionAbort=null;}_actionSent=false;_timerExpiredPending=false;_showActionLoading(false);document.querySelectorAll('#app .action-btn').forEach(b=>b.classList.remove('disabled'));startPolling();}},ACTION_SAFETY_TIMEOUT_MS)/* produto: safety se a API não responder; não é o timer de turno do servidor */;_lastAnimatedRoll=null;_initDiceAnimated=false;document.querySelectorAll('#app .action-btn').forEach(b=>b.classList.add('disabled'));_showActionLoading(true);stopTimer();stopPolling();if(isApiMode&&api){try{_actionAbort=new AbortController();window._actionAbort=_actionAbort;const result=await api.sendAction(actionData);_actionAbort=null;window._actionAbort=null;clearTimeout(_actionSentTimer);_showActionLoading(false);_actionSent=false;if(!result){showError('Sem resposta do servidor. Verifique sua conex\u00e3o.');return;}
console.info('[COMBAT:API] response phase=%s hasLR=%s hasPLR=%s hasFeed=%s',result.phase,!!result.lr,!!result.plr,!!(result.feed&&result.feed.length));_playCinematicResult(result,actionData.type);}catch(e){_actionAbort=null;window._actionAbort=null;clearTimeout(_actionSentTimer);_showActionLoading(false);_actionSent=false;console.error('[COMBAT:API] sendAction FAILED status=%s msg=%s',e.status||'?',e.message||e);/* Reset initiative prompt flag so player can re-roll after network failure */if(actionData.type==='initiative'){window._initPromptShown=false;}const msg=(e.status===401||e.status===403)?'Sessão expirada.':e.status===429?'Muitas ações. Aguarde um momento.':'Erro de conexão. Tente novamente.';showError(msg);if(e.status===429){setTimeout(()=>{document.querySelectorAll('#app .action-btn').forEach(b=>b.classList.remove('disabled'));startPolling();},RATE_LIMIT_RETRY_MS);}else{document.querySelectorAll('#app .action-btn').forEach(b=>b.classList.remove('disabled'));if(e.status!==401&&e.status!==403){startPolling();}}}}else{if(!tg){console.warn('[COMBAT] No Telegram WebApp',actionData);_actionSent=false;return;}
...
const hpColor=pct>60?'var(--v-hp-high-flat, #4caf50)':pct>25?'var(--v-hp-warn-flat, #f97316)':'var(--v-hp-crit-flat, #ef4444)';html+=`<div class="target-item" data-target="${i}">
        ...
        </div>`;});html+='<div class="skill-close" id="targetClose">Cancelar</div>';panel.innerHTML=html;panel.style.animation='none';panel.offsetHeight;panel.style.animation='';overlay.classList.add('active');_markCellsInRange(enemies);const _firstTarget=panel.querySelector('.target-item');if(_firstTarget)_firstTarget.focus();overlay.onkeydown=(e)=>{if(e.key==='Escape')_closeOverlay(overlay);};panel.onclick=(e)=>{const item=e.target.closest('.target-item');if(e.target.closest('#targetClose')){_closeOverlay(overlay);return;}
if(!item||_actionSent)return;vHaptic.medium();const target=parseInt(item.dataset.target, 10);_clearCellsInRange();overlay.classList.remove('active');if(actionType==='skill'||actionType==='bonus_use'){sendAction({type:actionType,skill_id:skillId,target:target});}else{sendAction({type:'attack',target:target});}};overlay.onclick=(e)=>{if(e.target===overlay)_closeOverlay(overlay);};}
function showItemPicker(items,enemies,allies){console.info('[COMBAT:UI_OPEN] picker=item items_count='+(items?items.length:0)+' enemies_count='+(enemies?enemies.length:0)+' allies_count='+(allies?allies.length:0));const panel=document.getElementById('itemPanel');const overlay=document.getElementById('itemOverlay');let html='<div class="skill-panel-title">🎒 Itens de Combate</div>';items.forEach(it=>{const isThrown=!!it.tdmg;const effText=isThrown?`${it.tdmg} ${it.ttype || ''}`:(it.heal?`Cura ${it.heal}`:'');const typeBadge=isThrown?'<span class="sk-aoe">Arremesso</span>':'<span class="sk-type sk-type-heal">Cura</span>';const itemCls=isThrown?'item-thrown':'item-heal';html+=`<div class="skill-item item-entry ${itemCls}" data-item="${escHtml(it.n)}" data-thrown="${isThrown}">
            <div>
